...
DWORD patternSearch(DWORD dwAddress, DWORD dwLen, BYTE* byteArray, char* szMask);
...
#define MASK "xx??" // the mask - make sure there are as many characters in this string as there are bytes in the pattern.
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD  ul_reason_for_call, LPVOID lpReserved) {
	if (ul_reason_for_call == DLL_PROCESS_ATTACH) { // we will be injecting the dll into the process
		...
		CreateThread(0, 0, (LPTHREAD_START_ROUTINE)&loop, 0, 0, 0); // multi-thread the loop function by passing in its address to WinAPI multithreading function
	}
	return TRUE;
}
...
// isPattern:
// takes an address in memory, a byte array to find, and a string of tokens denoting which bytes to ignore.
// searches through the byte array to find a match at that ***particular point in memory***
// goes through szMask to see which bytes to "ignore".

bool isPattern(const BYTE* pData, const BYTE* byteArray, const char* szMask)
{
	for (; *szMask; ++szMask, ++pData, ++byteArray) { // increment through the mask, data, and byteArray. They are all pointers so we can do this.
	...
	return (*szMask) == NULL; // will always return true when all bytes in the byte-array byteArray have been exhausted - this will be at the same time that all tokens in szMask have been exhausted
	// i.e., we have compared every single byte in the byte array and it has all matched in the memory. then szMask will point to nothing (hence, *szMask == NULL)
}

/* takes a start address dwAddress and a length dwLen //
runs a search from the start address for a pattern of bytes, the byte array byteArray //
...
(i.e., if we have byteArray (BYTE*)("\x40\x23\xB9\x1F") and are using the string mask "xx??",
...
=== Complexity remark: operates in O(nm) time in worst case; n is the length of the byte-array (byteArray), m is the length of memory we are searching (dwLen) ===
--- proof: for each memory address (there are m of them), we would make n comparisons (each byte
...
DWORD patternSearch(DWORD dwAddress, DWORD dwLen, BYTE* byteArray, char* szMask)
{
	for (DWORD i = 0; i < dwLen; i++) {
		if (isPattern((BYTE*)(dwAddress + i), byteArray, szMask)) { // isPattern determines if the instructions that start at a single address matches our array of bytes'
